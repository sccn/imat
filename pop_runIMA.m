...
if nargin ==1
    ...
    uilist = {{'style' 'text' 'string' 'Select ICs'}...
        { 'style' 'Checkbox'   'string' 'IC index' 'Tag' 'chbx_icindx', 'callback', cb_chbx_icindx 'Value' 1}...
        {'style'  'edit' 'string' ' ' 'tag' 'ed_icindx'}...
    ...
    geom = {{wt ht [0 0]  [1 1]} ...
        ...
        {wt ht [0.1 2]  [1 1]}  {wt ht [1 2] [1 2.5]} ...
        {wt ht [0.1 4]  [1 1]}...
        {wt ht [0 5]  [1 1]}  {wt ht [0.5 5] [0.8 1]}  {wt ht [1.6 5]  [1 1]}  {wt ht [2.1 5]  [0.8 1]} ...
        {wt ht [0 6]  [1 1]}  {wt ht [1 6] [2 1]} ...
        {wt ht [0.1 7]  [1 1]}...
        };
...
%     end
    ...
    LOGIC{1} = find((IND == 1) & (VAL > g.iclthreshold) & [EEGc.dipfit.model.rv]' < g.dipthreshold);
    else
    LOGIC{1} = find((IND == 1) & (VAL > g.iclthreshold)); % use only IC label classification
    end
    
    LOGIC{2} = find((IND == 2) & (VAL > g.iclthreshold)); % find muscle ICs with classification accuracy above threshold
    LOGIC{3} = find((IND == 3) & (VAL > g.iclthreshold)); % find eye ICs with classification accuracy above threshold
    LOGIC{4} = find((IND == 4) & (VAL > g.iclthreshold)); % find heart ICs with classification accuracy above threshold
